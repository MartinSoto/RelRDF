...
class SchemaBrowser(UiManagerSlaveDelegate):
    ...
    _classPixbuf = gtk.gdk.pixbuf_new_from_xpm_data(_classXpm)
    ...
    _propertyPixbuf = gtk.gdk.pixbuf_new_from_xpm_data(_propertyXpm)
    ...
    def addRdfClass(self, rdfClass):
        parent = self.classTS. \
                 append(None, [rdfClass, self._classPixbuf,
                               '<b>%s</b>' % str(rdfClass)])

        for p in rdfClass.getAllProperties():
            for r in p.range:
                self.classTS.append(parent,
                                    [p, self._propertyPixbuf,
                                     '<b>%s</b> %s' % (str(p), str(r))])
