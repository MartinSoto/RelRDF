...
class ModelBase(SyncMethodsMixin):
    ...
    def _cleanUpUses(self):
        ...
        cursor.execute("""
            ...
            WHERE connection = inet_client_port()
            """)
        cursor.execute("""
    ...
    _updateTimeTwoWay = string.Template(
        """
        DELETE FROM twoway_use_time WHERE version_a = $versionA AND version_b = $versionB;
...
#        ON DUPLICATE KEY UPDATE time = now()
    ...
    _createTwoWay = string.Template(
        ...
        INSERT INTO twoway (version_a, version_b, stmt_id, context)
          SELECT
            ...
            CASE sum(CASE WHEN v.version_id = $versionA THEN 1 ELSE 0 END +
                     CASE WHEN v.version_id = $versionB THEN 2 ELSE 0 END)
              WHEN 1 THEN 'A'
              WHEN 2 THEN 'B'
              WHEN 3 THEN 'AB'
            END AS context
    ...
    _connTwoWay = string.Template(
        ...
        VALUES ($versionA, $versionB, inet_client_port())
        """)
...
#        ON DUPLICATE KEY UPDATE time = now()
    ...
    _createThreeWay = string.Template(
        ...
        INSERT INTO threeway (version_a, version_b, version_c,
          ...
          SELECT
            ...
            v.stmt_id AS stmt_id,
            CASE
                sum(CASE WHEN v.version_id = $versionA THEN 1 ELSE 0 END +
                    CASE WHEN v.version_id = $versionB THEN 2 ELSE 0 END +
                    CASE WHEN v.version_id = $versionC THEN 4 ELSE 0 END)
              WHEN 1 THEN 'A'
              WHEN 2 THEN 'B'
              WHEN 3 THEN 'AB'
              WHEN 4 THEN 'C'
    ...
    _connThreeWay = string.Template(
        ...
        INSERT INTO threeway_conns (version_a, version_b, version_c,
                                    connection)
        VALUES ($versionA, $versionB, $versionC, inet_client_port())
        """)
